<?php 
	// var to store form data -- $_POST SUPER GLOBAL VAR 
	$form_data = ( ! empty( $_POST ) ) ? $_POST : null; 	
	// var to store is form complete
	$form_is_complete = ( empty( $_POST ) ) ? false : null;


	// function to check if user enter input in required field
	...
	// function to check if user enter proper input in email field
	...
	// function for form hendler -- sending email
	function send_message( $form_data ) {
		...
		$to = "Dusan Velimirovic <joe@casabona.org>";
		$from = "$name <$email>";
...
<?php 
	// EVALUATE IF FORM IS VALID AND PASS DATA TO send_message() FUNCTION
